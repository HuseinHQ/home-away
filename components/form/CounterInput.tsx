...
function CounterInput({ detail, defaultValue }: { detail: string; defaultValue?: number }) {
  ...
  const decreaseCount = () => setCount((prevCount) => (count > 0 ? prevCount - 1 : prevCount));
      ...
      <CardHeader className="flex flex-col gap-y-5">
        <div className="flex items-center justify-between flex-wrap">
          <div className="flex flex-col">
            ...
            <p className="text-muted-foreground text-sm">Specify the number of {detail.toLowerCase()}</p>
          </div>
